<template>
  <t-layout :class="`${prefix}-layout`">
    <t-tabs v-if="settingStore.isUseTabsRouter" drag-sort theme="card" :class="`${prefix}-layout-tabs-nav`"
      :value="$route.path" :style="{ position: 'sticky', top: 0, width: '100%', height: '25px' }"
      @change="handleChangeCurrentTab" @remove="handleRemove" @drag-sort="handleDragend">
    ...
    <t-content :class="`${prefix}-content-layout`">
      <l-breadcrumb v-if="settingStore.showBreadcrumb && !hideBreadcrumb" />
      <l-content />
    </t-content>
...
const hideBreadcrumb = computed(() => {
  return route.matched.some(match => match.meta?.hiddenBreadcrumb);
});
