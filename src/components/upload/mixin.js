...
import { AccessKeyId, AccessKeySecret, bucket, region, SecurityToken } from '/../../constants';
  ...
  methods: {
    ...
    handleClose() {
      ...
      if (this.isDoing) this.dialogVisible = false;
        ...
        this.$confirm('正在上传文件，关闭后上传文件会丢失！是否继续？', '操作提示', {
          confirmButtonText: '确 定',
          cancelButtonText: '取 消',
          type: 'warning',
        })
    ...
    async getOss() {
      // 请求后端接口，案例改为模拟数据
      // const res = await getToken(this.form);
      // let isPass = {
      //   pass: true,
      // };
      // if (res.status === 200) {
      ...
      this.credentials = { AccessKeyId, AccessKeySecret, SecurityToken };
      return isPass;
    },
    /**
     * 上传之前调用oss凭证
        ...
        if (percentage < 100 && raw.name.indexOf('.') !== -1) {
          item.client
            ...
            .then(() => {
              ...
              if (this.unList.length && this.uploadDisabled) this.resumeDisabled = false;
    ...
    async resumeMultipartUpload(item) {
      ...
      if (item) {
        ...
        Object.values(this.checkpoints).forEach((checkpoint) => {
          ...
          const index = this.fileList.findIndex((option) => option.upload === uploadId);
    ...
    async ossUpload(item) {
      ...
      try {
        ...
        if (percentage < 100 && raw.name.indexOf('.') !== -1) {
          const file = raw;
          const path = `${this.model ? `${this.model}/` : ''}${file.name}`;
          await item.client
    ...
    async multipartUpload() {
      ...
      for (const item of this.fileList) {
        ...
        if (!getOssRes.pass) {
          this.$message.error('获取oss上传凭证异常');
        }
        this.ossUpload(item, this.fileList);
      }
    },
    ...
    async onCreatePanFile(file) {
      const params = await this.getParams(file);
