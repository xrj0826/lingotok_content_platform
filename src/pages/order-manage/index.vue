        ...
        <div style="display: flex; flex-wrap: wrap; gap: 16px; margin-bottom: 16px;">
          ...
          <div style="display: flex; align-items: center;">
            ...
            <t-select v-model="searchSeriesName" placeholder="选择合集名称（可选）" style="width: 240px"
              :options="seriesNameOptions" clearable filterable allow-input />
          </div>
          ...
          <div class="video-preview-section">
            <div class="video-player-box">
              ...
              <div v-else-if="currentVideo.video?.video_cover_url" class="video-cover">
                <SafeMediaDisplay
                  :src="currentVideo.video.video_cover_url"
                  media-type="image"
                  alt="视频封面"
      ...
      <t-form ref="uploadFormRef" :data="uploadForm" :rules="ruleValidate">
        <t-form-item label="选择合集" name="series_name">
          <t-select v-model="uploadForm.series_name" placeholder="请选择合集" :options="seriesNameOptions" 
            clearable filterable allow-input />
        </t-form-item>
...
const handleVideoLoad = (event: Event) => {
  console.log('✅ [OrderManage] 视频加载成功:', currentVideo.value?.video?.video_trans_code_play_url || currentVideo.value?.video?.video_play_url);
  MessagePlugin.success('视频加载成功');
};

const handleVideoError = (error: string) => {
  console.error('💥 [OrderManage] 视频加载失败:', error);
  MessagePlugin.error(`视频加载失败: ${error}`);
};

const handleCoverLoad = (event: Event) => {
  console.log('✅ [OrderManage] 封面图片加载成功:', currentVideo.value?.video?.video_cover_url);
};

const handleCoverError = (error: string) => {
  console.error('💥 [OrderManage] 封面图片加载失败:', error);
  MessagePlugin.warning(`封面图片加载失败: ${error}`);
};
