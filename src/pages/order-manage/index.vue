      ...
      <template v-if="currentVideo">
        <div class="video-detail">
          ...
          <div class="video-preview-section">
            ...
            <div class="subtitle-section">
              <div class="subtitle-buttons">
                <t-button v-for="subtitle in getSubtitleList" :key="subtitle.language" variant="outline"
                  :theme="currentSubtitle?.language === subtitle.language ? 'primary' : 'default'"
              ...
              <div v-if="currentSubtitle" class="subtitle-editor">
                <div class="subtitle-header">
                  ...
                  <div class="subtitle-actions">
                    <t-button v-if="currentSubtitle.language === 'zh_draft'" theme="primary" size="small"
                ...
                <t-loading :loading="subtitleLoading">
                  <t-textarea v-model="subtitleContent"
                    ...
                    :autosize="{ minRows: 10, maxRows: 20 }" :readonly="currentSubtitle.language !== 'zh_draft'" />
                </t-loading>
              </div>
          ...
          <div class="video-info-section">
            ...
            <!-- 字幕草稿信息，仅在选中中文字幕草稿时显示 -->
            <div v-if="currentSubtitle?.language === 'zh_draft'" class="info-group">
              <div class="detail-item">
                <span class="label">修改人:</span>
                <span class="value">{{ currentVideo.video?.draft_subtitle_user || '-' }}</span>
              </div>
              <div class="detail-item">
                <span class="label">修改时间:</span>
                <span class="value">{{ formatTimestamp(currentVideo.video?.draft_subtitle_time) }}</span>
              </div>
              <div class="detail-item">
                <span class="label">字幕行数:</span>
                <span class="value">{{ currentSubtitleRows }}</span>
              </div>
            </div>
...
const columns = [
    ...
    cell: (h, { row }) => {
      ...
      return (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
          <t-tag theme={levelColorMap[level] || 'default'}>
            {!level ? '-' : level === '<UNSET>' ? '未设置' : level}
          </t-tag>
        </div>
      );
    }
  ...
  {
    colKey: 'series.interest_list',
    title: '合集interest列表',
    width: 200,
    ellipsis: true,
    cell: (h, { row }) => {
      const list = row?.series?.interest_list;
      return !list || !Array.isArray(list) || list.length === 0 ? '-' : list.join('; ');
    }
  },
  {
    colKey: 'video_status',
...
async function fetchVideoList() {
  ...
  try {
    ...
    console.log('Fetching video list with:', postData);
    ...
    if (res.data?.code === 200 && Array.isArray(res.data.data.video_list)) {
      ...
      console.log('Processed video list:', videos);
...
// 处理字幕列表显示
const getSubtitleList = computed(() => {
  if (!currentVideo.value?.video?.subtitle_list) return [];

  const subtitleList = [...currentVideo.value.video.subtitle_list];
  // 查找中文字幕
  const zhSubtitle = subtitleList.find(sub => sub.language === 'zh');
  ...
  if (zhSubtitle) {
    subtitleList.push({
      ...zhSubtitle,
      language: 'zh_draft',
      // 如果有草稿URL则使用草稿URL，否则使用中文字幕URL
      subtitle_play_url: currentVideo.value.video.draft_subtitle_url || zhSubtitle.subtitle_play_url
    });
  }
...
// 计算当前显示的字幕行数
const currentSubtitleRows = computed(() => {
  if (!subtitleContent.value) return 0;
  // 将字幕内容按行分割，并过滤掉空行
  return subtitleContent.value.split('\n').filter(line => line.trim() !== '').length;
});
...
const handleSubtitleSelect = async (subtitle) => {
  try {
    ...
    // 获取字幕内容
    let url = subtitle.subtitle_play_url;
    // 如果是中文字幕草稿，且没有草稿URL，则使用中文字幕的URL
    if (subtitle.language === 'zh_draft' && !currentVideo.value.video.draft_subtitle_url) {
      const zhSubtitle = currentVideo.value.video.subtitle_list.find(sub => sub.language === 'zh');
      if (zhSubtitle) {
        url = zhSubtitle.subtitle_play_url;
      }
    }
...
const handleSaveSubtitle = async () => {
  ...
  // 只允许修改中文字幕和中文字幕草稿
  if (currentSubtitle.value.language !== 'zh' && currentSubtitle.value.language !== 'zh_draft') {
    MessagePlugin.warning('只能修改中文字幕和中文字幕草稿');
    return;
  }
  ...
  try {
    ...
    const isDraft = currentSubtitle.value.language === 'zh_draft';
    const fileName = `${isDraft ? 'draft_' : ''}subtitle_${videoId}_${username}_${timestamp}.srt`;
    ...
    if (result.CommonMsg.Status === 200) {
      ...
      // 计算字幕行数
      const subtitleRows = subtitleContent.value.split('\n').filter(line => line.trim() !== '').length;
      ...
      const updateParams = {
        ...
        draft_subtitle_rows: subtitleRows // 添加字幕行数
      };
